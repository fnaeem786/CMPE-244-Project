...
CMD_HANDLER_FUNC(alarmHandler)
{
	...
	alarm_time_t time;
	if(cmdParams.beginsWith("set alarm"))
	{
		unsigned hr, mn, sc;
		if( 3 != cmdParams.scanf("%u %u %u ", &hr, &mn, &sc))
		{
			return false;
		}
		time.hour = hr;
		...
		alarmObj.setAlarm(time);

	}
	if(cmdParams == "disable alarm")
	{
		alarmObj.stopAlarm();
	}

	if(cmdParams == "get time")
	{
		time = alarmObj.getAlarmTime();
		printf("%u : %u : %u", time.hour, time.min, time.sec);
	}
	if(cmdParams.beginsWith("snooze"))
	{
		uint8_t userTime;
		if(1 != cmdParams.scanf("%u", &userTime))
		{
			return false;
		}
...
CMD_HANDLER_FUNC(shakerHandler)
{
	...
	//char pattern[16] = {0};
	vibration_t pattern;
	if(cmdParams.beginsWith("pattern"))
	{
		if(1 != cmdParams.scanf("%s", &pattern))
		{
			return false;
		}
...
CMD_HANDLER_FUNC(matrixHandler)
{
	...
	uint8_t brightness;
	if(cmdParams.beginsWith("brightness"))
	{
		if(1 != cmdParams.scanf("%u", &brightness))
		{
			return false;
		}
		matrixObj.setBrightness(brightness);
	}
	return true;
}
...
CMD_HANDLER_FUNC(audioHandler)
{
	...
	unsigned char value;
	if(cmdParams.beginsWith("play"))
	{
		if(1 != cmdParams.scanf("%c", &value))
		{
			return false;
		}
		audioObj.playAudio(value);
	}

	if(cmdParams == "stop")
	{
		audioObj.playAudio(0xFF);
	}
	return true;
}
...
{
	if(cmdParams == "get RTC")
	{
	printf("%i : %i : %i \n", (int)rtc_gettime().hour, (int)rtc_gettime().min, (int)rtc_gettime().sec);
	}
	return true;
}
