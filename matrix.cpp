...
bool matrix::init(uint8_t displayAddress, uint8_t dimmingRate, uint8_t blinkRate, I2C2 *i2c)
{
	...
	if(i2c->writeReg(displayAddress, 0x21, 0x21)){
		puts("Clock enabled");
	}


	//Set dimming frequency (brightness of displays)
	if(i2c->writeReg(displayAddress, dimmingRate, dimmingRate)){
		puts("Dimming set");
	}
	//Turn display on and set blink frequency
	if(i2c->writeReg(displayAddress, blinkRate, blinkRate)){
		puts("Blink frequency set");
	}



	printf("Initialized Display %i\n", displayAddress);
...
void matrix::writeDisplays(char string[], I2C2 *i2c)
{
	...
	for(int j=0; j<7; j++){
		...
		i2c->writeReg(disp.display1_addr,x,fonttable[string[0]][j] >> 1);
		i2c->writeReg(disp.display2_addr,x,fonttable[string[1]][j] >> 1);
		i2c->writeReg(disp.display3_addr,x,fonttable[string[2]][j] >> 1);
		i2c->writeReg(disp.display4_addr,x,fonttable[string[3]][j] >> 1);
		x+=2;
	}
}
...
void matrix::clearDisplay(I2C2 *i2c)
{
	...
	uint8_t x = 0x00;
	for(int j=0; j<7; j++){
		i2c->writeReg(disp.display1_addr,x,0x00);
		i2c->writeReg(disp.display2_addr,x,0x00);
		i2c->writeReg(disp.display3_addr,x,0x00);
		i2c->writeReg(disp.display4_addr,x,0x00);
		x+=2;
	}
